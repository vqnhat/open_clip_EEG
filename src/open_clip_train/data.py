...
class EEG_Dataset(Dataset):
    ...
    def __init__(self, transform=None, image_size=(224, 224), caption="Dummy caption", dataset_size=100, tokenizer=None,
                 is_train=True, fold=0, dataset='../data/2. BCI-ABSB/ABSB-4D/BCI_ABSB_SubjectB_2LR_4D.mat', max_EEG_channel=50):

        # self.tensor = data['WT']
        # self.labels = data['LABELS']
        self.preprocess_txt = lambda text: tokenizer(text)[0]
        self.is_train = is_train
        self.max_EEG_channel = max_EEG_channel
        self.dataset = dataset
        # data/1. BCI-3P/4C/BCI_3P_sub6_4D.mat')
        # data/2. BCI-ABSB/ABSB-4D/BCI_ABSB_SubjectB_2LR_4D.mat
        # data/3. BCI-III/Tensor-Form/4D-7-Channel/BTI_IV_ey_4D_7channels.mat')
        data = loadmat(dataset)

        if "BCI_JP" in dataset:
            N = data['WTrain'].shape[-1]
            p = np.arange(N).reshape(2, N // 2).T.ravel()
            data['WTrain'] = data['WTrain'][:,:,:,p]
            data['TrainLabels'] = data['TrainLabels'][p,:]

            N = data['WTest'].shape[-1]
            p = np.arange(N).reshape(2, N // 2).T.ravel()
            data['WTest'] = data['WTest'][:,:,:,p]
            data['TestLabels'] = data['TestLabels'][p,:]          

            self.tensor = np.concatenate((data['WTrain'], data['WTest']), axis=3)
            self.labels = np.concatenate((data['TrainLabels'], data['TestLabels']), axis=0)
        else:
            self.tensor = data['WT']
            self.labels = data['LABELS']
        
        if "BCI_IV" in dataset:
            N = self.tensor.shape[-1]
            p = np.arange(N).reshape(2, N // 2).T.ravel()
            self.tensor = self.tensor[:,:,:,p]
            self.labels = self.labels[p,:]

        #print(self.labels)
        num_sample = self.tensor.shape[-1]
        split = int(num_sample // 10)
        # fold = int(0)
        if self.is_train:
            self.tensor = np.concatenate(
                (self.tensor[:, :, :, 0:fold * split], self.tensor[:, :, :, (fold + 1) * split:]), axis=3)
            self.labels = np.concatenate((self.labels[0:fold * split, :], self.labels[(fold + 1) * split:, :]), axis=0)
            print("TRAIN DATA: !!!!!!!!!!!!!!!!", self.tensor.shape[-1])
        else:
            self.tensor = self.tensor[:, :, :, fold * split:(fold + 1) * split]
            self.labels = self.labels[fold * split:(fold + 1) * split, :]
            print("VAL DATA: !!!!!!!!!!!!!!!!", self.tensor.shape[-1])

    def __len__(self):
        return self.tensor.shape[-1]

    def __getitem__(self, idx):
        image = torch.tensor(self.tensor[:, :, :, idx]).float()
        image = (image - torch.min(image)) / (torch.max(image) - torch.min(image))
        # image = einops.rearrange(image, 'w (h c) -> (w c) h', h = 250, c = 4)
        # print("\\\\\\\\\\\\\\\\\\\\\\<<<<<<<<<<<<<<<< ", image.shape)
        if "BCI_ABSB" in self.dataset:
            image = image.permute(2, 0, 1)
        # image = image.unsqueeze(0)
        # image = einops.repeat(image, 'c w h -> (3 c) w h')
        image = image.unsqueeze(1)
        image = F.interpolate(image, size=(224, 224), mode='bilinear', align_corners=False)
        image = image.squeeze(1)
        #print("######################### ", image.shape)
        zeros_tensor = torch.zeros(self.max_EEG_channel-image.shape[0], image.shape[1], image.shape[2], dtype=image.dtype, device=image.device)
        image = torch.cat([zeros_tensor, image], dim=0)
        #print("######################### ", image.shape)
        caption = "left" if self.labels[idx] == 1 else "right"
        return {"image": image, "text": self.preprocess_txt(caption), "raw_text": caption}


def get_EEG_dataset(args, preprocess_fn, is_train, epoch=0, tokenizer=None, naflex_data_config=None, fold=0):
    ...
    dataset = EEG_Dataset(
        ...
        max_EEG_channel=args.max_EEG_channel,
    )
    num_samples = len(dataset)
    sampler = DistributedSampler(dataset) if args.distributed and is_train else None
    shuffle = is_train and sampler is None
...
def get_data(args, preprocess_fns, epoch=0, tokenizer=None, naflex_data_config=None):
    ...
    if args.train_data or args.dataset_type in ("EEG"):
    ...
    if args.val_data or args.dataset_type in ("EEG"):
