...
def get_clip_metrics(image_features, text_features, logit_scale, all_text):
    left_text_feature = right_text_feature = None
    ground_truth = torch.arange(len(text_features)).view(-1, 1)
    P = N = 0
    for idx, text in enumerate(all_text):
        if text == "left":
            left_text_feature = text_features[idx]
            ground_truth[idx] = 0
            P = P + 1
        else:
            right_text_feature = text_features[idx]
            ground_truth[idx] = 1
            N = N + 1

    text_features = torch.stack([left_text_feature, right_text_feature])
    # print("*************** ", ground_truth)
    metrics = {}
    ...
    for name, logit in logits.items():
        ...
        pre_TP_0 = torch.sum(ranking[:, 0] == 0).detach().cpu().numpy()
        TP_0 = torch.sum(torch.logical_and((ranking[:, 0] == 0), (ground_truth[:, 0] == 0))).detach().cpu().numpy()
        pre_TP_1 = torch.sum(ranking[:, 0] == 1).detach().cpu().numpy()
        TP_1 = torch.sum(torch.logical_and((ranking[:, 0] == 1), (ground_truth[:, 0] == 1))).detach().cpu().numpy()
        for k in [1]:
            metrics[f"{name}_R@{k}"] = np.mean(preds < k)
        precision_0 = 0 if pre_TP_0 == 0 else TP_0 / pre_TP_0
        recall_0 = TP_0 / P
        precision_1 = 0 if pre_TP_1 == 0 else TP_1 / pre_TP_1
        recall_1 = TP_1 / N
        precision = (precision_0 + precision_1) / 2
        recall = (recall_0 + recall_1) / 2
        FP = pre_TP_0 - TP_0
        FN = pre_TP_1 - TP_1
        MCC = ((TP_0 * TP_1) - (FP * FN)) / np.sqrt(pre_TP_0 * (TP_0 + FN) * (TP_1 + FP) * pre_TP_1)
...
def compute_t_sne(features, epoch, output_dir):
    # ---------------------------------------------------------
    # METHOD C: t-SNE Clustering in Joint Space
    # ---------------------------------------------------------
    # Stack them together to project them into the exact same 2D space
    eeg_features = features["image_features"]
    text_features = features["text_features"]
    print("#################### ", eeg_features.shape)
    all_features = np.vstack((eeg_features, text_features))

    # Create labels for the legend
    tsne_labels = features["all_text"]
    eeg_labels = []
    for label in tsne_labels:
        label = 'EEG ' + label
        eeg_labels.append(label)
    
    tsne_labels = eeg_labels + tsne_labels    

    # Use t-SNE (you can also use PCA from sklearn.decomposition)
    tsne = TSNE(n_components=2, perplexity=30, random_state=42)
    embeddings_2d = tsne.fit_transform(all_features)

    plt.figure(figsize=(8, 6))
    colors = {'EEG left': 'blue', 'EEG right': 'red', 'left': 'cyan', 'right': 'orange'}
    markers = {'EEG left': 'o', 'EEG right': 'o', 'left': 'X', 'right': 'X'}

    for label in np.unique(tsne_labels):
        idx = np.array(tsne_labels) == label
        plt.scatter(embeddings_2d[idx, 0], embeddings_2d[idx, 1], 
                    c=colors[label], marker=markers[label], label=label, alpha=0.7, s=80)
        
    plt.title("Epoch %d t-SNE Joint Visualization of EEG and Text Features" %(epoch))
    plt.legend()
    os.makedirs(output_dir, exist_ok=True)

    plt.savefig(os.path.join(output_dir, "epoch_%d_tsne.png" % (epoch)))

def compute_similarity_heatmap(features, epoch, output_dir):
    eeg_features_left = []
    text_features_left = []
    eeg_features_right = []
    text_features_right = []
    ...
    for idx, label in enumerate(features['all_text']):
        if 'left' in label:
            eeg_features_left.append(features['image_features'][idx])
    ...
    similarity_matrix = cosine_similarity(eeg_features, text_features)

    plt.figure(figsize=(8, 6))
    sns.heatmap(similarity_matrix, cmap='viridis')
    plt.title("Sample-to-Sample Cosine Similarity\n(EEG Features vs Text Features)")
...
def evaluate(task, data, epoch, args, tb_writer=None, tokenizer=None):
    ...
    if 'val' in data and (args.val_frequency and ((epoch % args.val_frequency) == 0 or epoch == args.epochs)):
        ...
        all_primary_features, all_text_features, all_text = [], [], []
    ...
    return metrics, {"image_features": torch.cat(all_primary_features), "text_features": torch.cat(all_text_features), "all_text": all_text}
